...
const INVENTORY_DEFAULT = { basicAvatars: [], premiumAvatars: [], basicPets: [], premiumPets: [] };

const AdminPortal = () => {
  const [shopForm, setShopForm] = useState(SHOP_DEFAULT);
  const [updateForm, setUpdateForm] = useState(UPDATE_DEFAULT);
  const [shopItems, setShopItems] = useState([]);
  const [shopInventory, setShopInventory] = useState(() => ({ ...INVENTORY_DEFAULT }));
  ...
  const loadContent = async () => {
    ...
    try {
      ...
      setShopInventory(data?.shop?.inventory || { ...INVENTORY_DEFAULT });
  ...
  const activeShopInventory = shopInventory;
