...
export default function AdminPanel() {
  ...
  const loadCancellations = async () => {
    try {
      const res = await fetch(`/api/admin/get-data?dataType=cancellations&adminEmail=${user.email}`);
      const data = await res.json();
      if (data.success) {
        setCancellationsList(data.data);
      }
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-900 via-blue-900 to-indigo-900">
      ...
      {/* Message Banner */}
      {message.text && (
        <div className={`container mx-auto px-4 py-4 mt-4 rounded-lg ${message.type === 'success' ? 'bg-green-500' : 'bg-red-500'
      ...
      <div className="container mx-auto px-4 py-6">
        <div className="flex space-x-4 mb-6">
          {['avatars', 'pets', 'updates', 'cancellations'].map(tab => (
            <button
              key={tab}
              onClick={() => setActiveTab(tab)}
              className={`px-6 py-3 rounded-lg font-semibold capitalize transition-all ${activeTab === tab
                  ? 'bg-white text-purple-900 shadow-lg'
                  : 'bg-purple-800 text-white hover:bg-purple-700'
                }`}
            >
              {tab === 'avatars' && '👤 '}{tab === 'pets' && '🐾 '}{tab === 'updates' && '📢 '}{tab === 'cancellations' && '🚫 '}
              {tab}
            </button>
          ))}
        </div>
        ...
        <div className="bg-white rounded-xl shadow-2xl p-8">
                ...
                {avatarList.map(avatar => (
                  <div key={avatar.id} className="border border-gray-200 rounded-lg p-4 hover:shadow-lg transition-all">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className={`px-2 py-1 rounded text-xs font-semibold ${avatar.type === 'premium' ? 'bg-yellow-200 text-yellow-800' : 'bg-blue-200 text-blue-800'
                        }`}>
                        {avatar.type}
                      </span>
                    </div>
                ...
                {petList.map(pet => (
                  <div key={pet.id} className="border border-gray-200 rounded-lg p-4 hover:shadow-lg transition-all">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className={`px-2 py-1 rounded text-xs font-semibold ${pet.type === 'premium' ? 'bg-yellow-200 text-yellow-800' : 'bg-blue-200 text-blue-800'
                        }`}>
                        {pet.type}
                      </span>
                    </div>
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <div className="flex items-center space-x-2 mb-2">
                          <span className={`px-3 py-1 rounded text-sm font-semibold ${update.type === 'feature' ? 'bg-green-200 text-green-800' :
          ...
          {activeTab === 'cancellations' && (
            <div>
              <h2 className="text-2xl font-bold text-gray-800 mb-6">Cancelled & Deleted Accounts ({cancellationsList.length})</h2>
                ...
                <table className="min-w-full bg-white border border-gray-200 rounded-lg">
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {cancellationsList.length === 0 ? (
                      ...
                      cancellationsList.map((user) => (
                        <tr key={user.id} className="hover:bg-gray-50">
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="text-sm font-medium text-gray-900">{user.email}</div>
                            <div className="text-xs text-red-500">{user.accountStatus}</div>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="text-sm text-gray-500">
                              {new Date(user.canceledAt).toLocaleDateString()}
                            </div>
                            <div className="text-xs text-gray-400">
                              {new Date(user.canceledAt).toLocaleTimeString()}
                            </div>
                          </td>
                          <td className="px-6 py-4">
                            <div className="text-sm text-gray-900 font-medium">
                              {user.cancelReason || 'No reason provided'}
                            </div>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            {user.cancellationDetails?.cancellationMethod || 'Unknown'}
                            {user.cancellationDetails?.stripeCanceled && (
