// components/curriculum/sections/HistorySection.js
// Lazy-loaded History section for Curriculum Corner
...
const historyActivities = [
    {
        id: 'timeline-builder', name: 'Timeline Builder', icon: '📅',
    ...
    {
        id: 'ancient-civilizations', name: 'Ancient Civilizations', icon: '🏛️',
        description: 'Explore ancient cultures and societies',
        component: ComingSoon
    }
];

const HistorySection = ({ onBack, showToast, students }) => {
            ...
            <div className="space-y-6">
                <div className="bg-white rounded-xl shadow-sm p-4 flex items-center justify-between">
                    <div className="flex items-center gap-2 text-slate-600">
                        ...
                        <span>→</span>
                        <button onClick={() => setActiveActivity(null)} className="hover:text-blue-600">History</button>
                        <span>→</span>
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-gradient-to-r from-amber-500 to-orange-600 text-white rounded-2xl p-8 text-center">
                <div className="text-5xl mb-4">🏺</div>
                <h3 className="text-2xl font-bold mb-2">History Tools Coming Soon!</h3>
                <p className="opacity-90">We're building timeline builders and historical explorers for your classroom.</p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                {historyActivities.map(activity => (
                    <button key={activity.id} onClick={() => setActiveActivity(activity)}
                        className="bg-white rounded-xl shadow-sm p-5 text-left hover:shadow-lg transition-all hover:scale-[1.02] border border-slate-200 hover:border-amber-300 relative opacity-60">
                        ...
                        <div className="flex justify-between items-center">
                            ...
                            <span className="text-amber-600 font-semibold text-sm">Preview →</span>
                        </div>
                    </button>
                ))}
            </div>
