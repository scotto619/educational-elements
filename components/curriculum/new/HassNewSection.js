...
import { hassDomains } from '../../../utils/curriculumData';

const HassNewSection = ({ onBack, onToggleAssign, assignedTopics }) => {
    ...
    return (
        <div className="space-y-12 animate-in fade-in duration-500 pb-12 max-w-7xl mx-auto">
            {/* Header */}
            <div className="bg-white rounded-3xl shadow-lg p-8 flex flex-col md:flex-row md:items-center justify-between gap-6 border-t-4 border-t-amber-500 relative overflow-hidden">
                ...
                <div>
                    <button
                        onClick={onBack}
                        className="text-slate-400 hover:text-amber-600 mb-2 flex items-center gap-1 text-sm font-semibold transition-colors"
                    >
                        ← Back to Hub
                    </button>
                    <div className="flex items-center gap-4">
                        <span className="text-5xl bg-amber-100 p-4 rounded-2xl shadow-inner">🌍</span>
                        <div>
                            <h2 className="text-4xl font-black text-slate-800 tracking-tight">HASS Framework</h2>
                        ...
                        <div className="p-6 grid grid-cols-1 sm:grid-cols-2 gap-4 flex-grow bg-slate-50/50">
                            {domain.topics.map(topic => (
                                <button
                                    ...
                                    className="bg-white rounded-xl p-4 border border-slate-200 hover:border-amber-300 hover:shadow-md transition-all group flex items-start gap-4 text-left"
                                >
                                    <span className="text-3xl group-hover:scale-110 transition-transform">{topic.icon}</span>
                                    <div>
                                        <h4 className="font-bold text-slate-800 text-sm group-hover:text-amber-700 transition-colors line-clamp-2 leading-tight">
                                            {topic.name}
                                        </h4>
                                        <span className="text-xs text-slate-400 mt-1 block group-hover:text-amber-500 font-medium">Explore →</span>
                                    </div>
                                </button>
                            ))}
                        </div>
