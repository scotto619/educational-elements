...
const StudentsTab = ({ 
  ...
  return (
    <div className="flex gap-8 animate-fade-in">
      ...
      {/* Right Side - Students Management */}
      <div className="w-2/3">
        ...
        {students.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {students.map((student) => {
              // Calculate quest completion for this student
              const dailyCompleted = dailyQuests.filter(q => 
                q.category === 'individual' && checkQuestCompletion(q.id, student.id)
              ).length;
              const weeklyCompleted = weeklyQuests.filter(q => 
                q.category === 'individual' && checkQuestCompletion(q.id, student.id)
              ).length;
              const totalQuests = dailyQuests.filter(q => q.category === 'individual').length + 
                                weeklyQuests.filter(q => q.category === 'individual').length;
                ...
                >
                  ...
                  <div className="mb-4">
                    ...
                    <div className="text-xs text-gray-600">
                      Quest Progress: {totalCompleted}/{totalQuests}
                    </div>
                  </div>
