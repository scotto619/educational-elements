// components/quizshow/student/StudentGameView.js - STUDENT QUIZ INTERFACE
import React, { useState, useEffect } from 'react';
import { ref, update } from 'firebase/database';
import { database } from '../../../utils/firebase';
import { calculateQuizScore, playQuizSound, formatScore } from '../../../utils/quizShowHelpers';
        ...
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">Score: {formatScore(score)}</p>
            </div>
          </div>
      ...
      <div className="max-w-4xl mx-auto p-6">
            ...
            {questionPhase === 'answering' && !hasAnswered && (
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                <p className="text-blue-800 font-semibold">
                  Choose your answer quickly to earn bonus points!
                </p>
              </div>
            )}
          </div>
            ...
            {currentQuestion.options?.map((option, index) => {
              ...
              if (questionPhase === 'answering' && !hasAnswered) {
                ...
                if (index === selectedAnswer) {
                  buttonStyle += index === currentQuestion.correctAnswer 
                    ? ' bg-green-500 ring-4 ring-green-300' 
                    : ' bg-red-500 ring-4 ring-red-300';
                } else if (index === currentQuestion.correctAnswer && questionPhase === 'results') {
        ...
        {hasAnswered && (
          <div className={`rounded-2xl shadow-xl p-8 text-center text-white ${
            ...
            {isCorrect ? (
              <div>
                <p className="text-xl mb-2">You earned {formatScore(pointsEarned)} points!</p>
                <p className="text-lg opacity-90">Great job! 🌟</p>
              </div>
            ) : (
              <div>
                <p className="text-xl mb-2">No points this time</p>
